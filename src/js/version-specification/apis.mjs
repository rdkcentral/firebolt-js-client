...
const run = async (version, parsedArgs) => {
    ...
    return new Promise( async (resolve, reject) => {
        ...
        while (legacy>major-parsedArgs['legacy-versions']-1 && legacy>0) {
            ...
            await exec(`cd build; mkdir temp${legacy}; cd temp${legacy}; npm init -y ; npm install --saveDev @firebolt-js/core-client@${v}`)

            version.apis[`${legacy}`] = await loadJson(`./build/temp${legacy}/node_modules/@firebolt-js/core-client/dist/firebolt-open-rpc.json`)
