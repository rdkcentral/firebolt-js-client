...
static JSCValue *builder_cb(gpointer user_data) {
  ...
  JSCValue *transport = create_transport(ctx, config->fireboltEndpoint.c_str(),
                                         config->enableDebug);
  ...
  JSCValue *builderResult = jsc_value_function_call(builder, JSC_TYPE_VALUE,
                                                    builderOpts, G_TYPE_NONE);
  if (!builderResult) {
...
static bool setup_bridge_script(JSCContext *ctx, WebKitWebPage *page,
                                gpointer userData) {
  ...
  JSCValue *builder_fn = jsc_value_new_function(
      ctx, "get", G_CALLBACK(builder_cb), userData,
  ...
  JSCValue *serviceManagerBuilderResult =
      jsc_value_function_call(builderObj, JSC_TYPE_VALUE, builder, G_TYPE_NONE);
 ...
 */
static void onWindowObjectCleared(WebKitScriptWorld *world, WebKitWebPage *page,
                                  WebKitFrame *frame, gpointer userData) {
...
webkit_web_extension_initialize_with_user_data(WebKitWebExtension *extension,
  ...
  GVariant *injectedSettings =
      g_variant_lookup_value(userData, "firebolt", G_VARIANT_TYPE_VARDICT);
  ...
  struct FireboltExtensionConfig *config =
      g_new0(struct FireboltExtensionConfig, 1);

  if (!fireboltEndpoint || fireboltEndpoint[0] == '\0') {

    // Try to read environment variable for FIREBOLT_ENDPOINT if available
    const char *fireboltEndpointFromEnv = getenv("FIREBOLT_ENDPOINT");

    if (!fireboltEndpointFromEnv || fireboltEndpointFromEnv[0] == '\0') {
      g_critical("FIREBOLT_ENDPOINT Not available exiting");
      return;
    } else {
      // Set the firebolt endpoint from env variable
      config->fireboltEndpoint = g_strdup(fireboltEndpointFromEnv);
      // given fireboltEndpointFromEnv is from getenv, it's already a constant
      // string
    }
  } else {
    // Set the firebolt endpont from settings
