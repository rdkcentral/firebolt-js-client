...
JSCValue *evaluate_bridge_script(JSCContext *jsContext) {
  ...
  GBytes *bytes =
      g_resources_lookup_data("/org/rdk/browser/extensions/firebolt-bridge.js",
                              G_RESOURCE_LOOKUP_FLAGS_NONE, &error);
  if (bytes) {
    gsize sz;
    const void *ptr = g_bytes_get_data(bytes, &sz);
    if (ptr && sz) {
      jsc_context_push_exception_handler(jsContext, print_exception, NULL,
                                         NULL);
      JSCValue *script =
          jsc_context_evaluate(jsContext, (const char *)(ptr), sz);
    ...
    g_warning("failed to load firebolt bridge script from resources, %s",
              error->message);
    g_error_free(error);
    error = nullptr;
  }
...
JSCValue *evaluate_builder_script(JSCContext *jsContext) {
  GError *error = nullptr;
  GBytes *bytes =
      g_resources_lookup_data("/org/rdk/browser/extensions/firebolt-builder.js",
                              G_RESOURCE_LOOKUP_FLAGS_NONE, &error);
  if (bytes) {
    gsize sz;
    const void *ptr = g_bytes_get_data(bytes, &sz);
    ...
    g_warning("failed to load firebolt builder script from resources, %s",
              error->message);
    g_error_free(error);
    error = nullptr;
  }
...
JSCValue *create_transport(JSCContext *jsContext, const char *url,
  ...
  return jsc_value_new_object(jsContext,
