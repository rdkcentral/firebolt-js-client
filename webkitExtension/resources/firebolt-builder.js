(function() {
	...
	return function({
		...
		window.___fireboltTransport___ = transport;

		console.log("Checking transport methods " + typeof transport);
		for (var key in transport) {
			console.log("transport key " + key );
		}
		...
		return {
			build: function() {
				...
				if (!_connecting) {
					if (!_transportSet) {
						console.log("Setting up transport listeners");
