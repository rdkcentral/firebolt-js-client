...
void WebSocketTransport::on_message(GBytes* message)
{
    ...
    if (_on_message_cb )
    {
        ...
        JSCValue* value = jsc_value_new_string(jsc_value_get_context(_on_message_cb), data);
        if (value) {
...
static void call_open_callback(BaseTransport* transport)
{
    if (!transport) {
...
static void call_send_callback(BaseTransport* transport, const char* message)
{
    transport->send(message);
}

static void call_close_callback(BaseTransport* transport)
{
    transport->close();
}
...
JSCClass* create_transport_class(JSCContext* jsContext)
{
    ...
    jsc_class_add_method(
        transport_class,
        "close",
        G_CALLBACK(call_close_callback),
        nullptr,
        nullptr,
        G_TYPE_NONE,
        0,
