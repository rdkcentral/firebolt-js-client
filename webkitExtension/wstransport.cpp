...
ExceptionHandler::ExceptionHandler(JSCContext* context)
        : _context(context)
    {
...
void WebSocketTransport::on_message(GBytes* message)
{
    ...
    JSCValue* value = jsc_value_new_string(jsc_context_get_current(), data);

    if (_on_message_cb)
    {
        ExceptionHandler handler{jsc_value_get_context(_on_message_cb)};
        auto result = jsc_value_function_call(_on_message_cb, JSC_TYPE_VALUE, value, G_TYPE_NONE);
        if (result) {
            g_object_unref(result);
        }
    }
...
{
    g_message("Setting onopen callback\n");
